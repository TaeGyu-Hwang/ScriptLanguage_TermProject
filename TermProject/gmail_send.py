...
def sendMail(fromAddr, toAddr, msg):    # SMTP 연결로 메일을 보내는 함수
    ...
    try:
        ...
        # 앱 비밀번호 이용
        s.login('hwg4906@gmail.com', 'kozp kuzf zpig atyd')  # 여기에 앱 비밀번호를 입력하세요.
...
def onEmailInput():
    ...
    if '@' in addrEmail and addrEmail != '':
        ...
        except Exception as e: # 예외처리: 메일 보내기에 실패했을 때
            msgbox.showerror("Error", f"유효하지 않은 메일주소입니다. 에러: {e}")
            inputEmail.focus_set()
            inputEmail.delete(0, 'end')
...
def onEmailPopup():
    ...
    # 예외처리: 사용자가 동물병원을 선택하지 않고, 버튼을 누를 경우
...
if __name__ == '__main__':
    print("gmail_send.py runned\n")
else:
    print("gmail_send.py imported\n")
