...
def onGraphPopup():     # 런처에서 그래프 버튼을 누르면 실행되는 함수
    ...
    popup.title("경기도 동물병원 현황 내 시군별 동물병원 그래프")
...
def getData():  # REST API에서 데이터를 가져와 시군별 병원의 개수를 세는 함수
    ...
    base_url = "https://openapi.gg.go.kr/Animalhosptl?pSize=1000&KEY=" + key + "&pIndex="

    server.hList = [0 for _ in server.city_list]  # 데이터를 초기화
...
def onMapPopup(city):  # 바 클릭 시 실행되는 함수
    # 지도 팝업을 띄움
    # 선택한 시군 내에 있는 동물병원(지도 정보가 포함된 동물병원에 한해) 위치 출력
    global map_popup
    map_popup = Toplevel()
    map_popup.geometry("800x600+100+100")
    map_popup.title("<" + city + "> 내 동물병원")
    ...
    while True:
        ...
        for item in elements:  # 'row' element들
            ...
            if item.find('SIGUN_NM').text == city and getStr(item.find('REFINE_WGS84_LAT').text) != '정보없음' and getStr(item.find('REFINE_WGS84_LOGT').text) != '정보없음':
                ...
                marker_1 = map_widget.set_position(lat, logt, marker=True, marker_color_outside="grey", marker_color_circle="white")  # 위도,경도 위치지정
